...
class TestAir17PushAckAndDegradation(unittest.TestCase):
    ...
    def test_host_gateway_hub_push_and_ack(self, mock_urlopen):
        ...
        hub.board_cellular_data = True  # 关键：开启蜂窝数据时不得退让！
        hub.notify_config = {
            "feishu": {
                "enable": 1,
        ...
        written_frames = []
        hub._send_serial_cmd = lambda cmd_dict: written_frames.append(cmd_dict) or True
        ...
        self.assertTrue(any(f.get("cmd") == "notify_ack" and f.get("data", {}).get("status") == "ok" for f in written_frames),
