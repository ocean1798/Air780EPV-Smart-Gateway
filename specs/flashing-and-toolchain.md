...
## 5. 合宙官方固件尾号深度解析（103 vs 109 避坑实录）

在移芯二代 EC718 架构（如 `Air780EPM`）中，合宙官方发布了多个不同后缀的 `V2050` 固件。其本质是官方为了适应极端资源限制所做的**编译期功能剪裁**：

| 固件包文件名 | 固件编号 | 脚本空间 | C 核心库中是否含 `sms` | 官方定制定位与核心用途 |
| :--- | :---: | :---: | :---: | :--- |
| **`LuatOS-SoC_V2050_Air780EPM_103.soc`** | **`103`** | **448 KB** | **✅ 完整内置** | **【标准全能版·本项目唯一指定】** 预留最大 Lua 脚本存储空间，保留完整短信与外设库，专用于智能网关与物联网通信。 |
| **`LuatOS-SoC_V2050_Air780EPM_109.soc`** | **`109`** | **288 KB** | ❌ **彻底移除** | **【随身 WiFi 免驱网卡版】** 专用于插在电脑上纯当 4G 网卡上网。为了塞入大吞吐量虚拟网卡驱动与网络缓存，原厂剔除了 `sms` 库。 |
| **`LuatOS-SoC_V2050_Air780EPM_111.soc`** | **`111`** | **384 KB** | ⚠️ 特化裁剪 | **【定制功能版】** 针对特定行业客户裁剪。 |
| **`LuatOS-SoC_V2050_Air780EPM_1.soc`** | **`1`** | **384 KB** | ✅ 包含 | **【全量完整包】** 包含全功能但脚本空间中等。 |

### 5.1 错刷 109 固件引发死锁的实机调用栈复盘
当在 Air780EPM 上错刷 109 固件时：
1. **不插 SIM 卡**：模组不触发搜网与短信上报流程，表现看似正常；
2. **插入 SIM 卡后**：网络驻网就绪（`SIM_IND: READY`），系统立即尝试执行 `sms_service.lua`：
   ```lua
   if sms.autoLong then sms.autoLong(true) end
   ```
3. **崩溃与死锁**：109 底包中没有 `sms` 变量（为 `nil`），Lua 虚拟机直接报错退出：
   ```text
   E/main sms_service.lua:110: attempt to index a nil value (global 'sms')
   E/main Lua VM exit!! reboot in 15000ms
   ```
4. **外在表现**：底层触发硬件看门狗复位，USB 频繁断开重连，电脑端不断跳出 `[WinError 22] 设备不识别此命令`，陷入无限死锁。
5. **结论**：**智能通信网关必须严守 `103` 标准底包！**

---

## 6. 硬件开机改线与底层强刷救砖标准工序

### 6.1 芯片出厂光刻 BootROM 强刷救砖 (S1 / BOOT)
移芯平台芯片（EC718/EC618）内部固化了不可被抹除的硬件 BootROM。无论板载 Flash 中的系统或 Bootloader 损坏到何种程度，均可通过以下物理操作无损救活：
1. 完全拔下模组 Type-C 数据线；
2. **长按住板载的 `S1`（BOOT）按键不放**；
3. 将 Type-C 线重新插入电脑 USB 口；
4. 此时芯片内部拉低 BOOT 逻辑，直接切入物理 BootROM 模式（设备管理器识别为 `VID_17D1 & PID_0001` 或专有下载端口）；
5. 松开 `S1` 按键，在 Luatools 中选择正确的官方标准底包（如 `V2050_103`），点击【下载底层和脚本】，即可重新灌入完整内核，彻底复活板卡。

### 6.2 POWKEY 开机脚接地安全规范与通电自启改线 (S2 / POW)
- **一代开发板特性**：`EVB_Air780X_V1.5`（EC618）插上 USB 默认处于待机关机态，需长按 S2 键 1.5 秒启动。
- **改线原理**：
  模组引脚 7（`POWKEY`）在内部自带弱上拉到 VBAT，且板载回路设计有 **5.6kΩ 限流保护电阻**。外部将 POWKEY 直接接地属于合宙官方认证的安全操作，不会造成电源短路或烧毁引脚。
- **自启动改装方案**：
  - 方案 A（无焊烙铁）：使用导线将 `S2` 按键的对角两引脚短接；
  - 方案 B（标准工整）：在开发板背面的 `R6` 贴片焊盘上补上一颗 0 欧姆电阻或焊锡直接短接。
  - **改装效果**：插入充电头或电脑 USB 即可直接开机联网，完美满足免人工看护的随身智能网关需求。
